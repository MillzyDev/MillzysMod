...
foreach ($fileName in $modFiles) {
    if ($useDebug -eq $true) {
        & adb push build/debug/$fileName /sdcard/Android/data/com.stresslevelzero.bonelab/files/mods/$fileName
    } else {
        & adb push build/$fileName /sdcard/Android/data/com.stresslevelzero.bonelab/files/mods/$fileName
    }
}
